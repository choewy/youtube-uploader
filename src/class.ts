...
import {
  ClickSelectedChannelError,
  ClickUploadButtonError,
  OpenYoutubeStudioError,
  InsertEmailError,
  InsertPasswordError,
  NotFoundChannelNameError,
  OpenChannelSelectMenuError,
  OpenTopAccountMenuError,
  UploadFileError,
  WriteVideoDescriptionError,
} from './errors';
...
export class YoutubeUploader {
  ...
  async sign(credentials: Credentials, options?: PuppeteerLaunchOptions) {
  ...
  private async open(options?: PuppeteerLaunchOptions): Promise<void> {
    ...
    try {
      ...
      throw new OpenYoutubeStudioError(e);
    }
  }
  ...
  private async insertPassword() {
    try {
      ...
      throw new InsertPasswordError(e);
    }
  }
  ...
  private async openTopAccountMenu() {
    try {
      ...
      throw new OpenTopAccountMenuError(e);
    }
  }
  ...
  private async openChannelSelectMenu() {
    try {
      ...
      throw new OpenChannelSelectMenuError(e);
    }
  }
  ...
  private async clickSelectedChannel() {
    try {
      ...
      if (nth === 0) {
        throw new NotFoundChannelNameError();
      }
      ...
      throw new ClickSelectedChannelError(e);
    }
  }
  ...
  private async clickUploadButton(): Promise<void> {
    try {
      ...
      throw new ClickUploadButtonError(e);
    }
  }
  ...
  private async uploadFile(path: string): Promise<void> {
    try {
      ...
      throw new UploadFileError(e);
    }
  }

  private async writeVideoDescription(video: Video) {
    try {
      let url = '';
      ...
      throw new WriteVideoDescriptionError(e);
    }
  }
}
